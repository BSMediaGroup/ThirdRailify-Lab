...
export async function streamResearch(env,request,auth,b,preferences,context){
  ...
  const run=(async()=>{try{
    ...
    const metadata={...result,text:undefined,files:undefined,artifacts,cleanup:result.cleanup?.length?'Provider file cleanup is pending; recovery will retry.':null};await persist(result.completed?'completed':'partial',metadata);await recordUsage(env,{idempotencyKey:`chat:${runId}`,ownerId:auth.owner,projectId:b.projectId,conversationId:runId,provider:b.provider,profileId:credential.profileId,fingerprint:credential.fingerprint,model:b.model,operation:'research_chat',outcome:result.completed?'succeeded':'partial',...usageFromResponse(result.usage||{}),toolCalls:result.tools?.length||0,...providerCostFields(result),metadata:{sources:result.sources?.length||0,artifacts:artifacts.length}});emit('done',metadata);
