...
import {externalAttachment} from './stock-media.mjs';
...
export async function streamResearch(env,request,auth,b,preferences,context){
  ...
  const db=dbOf(env),runId=id(),credential=await resolveCredential(env,auth.owner,b.provider,b.model,b.profileId||''),provider=new Providers(()=>env,globalThis.fetch,{[b.provider]:credential}),profile=normalizeProfile(preferences.researchProfiles?.[key]);provider.key(b.provider);
  ...
  const run=(async()=>{try{
    ...
    const result=await provider.chat(b.provider,b.messages,b.model,delta=>{text+=delta;emit('delta',{text:delta});if(Date.now()-lastSave>2000){lastSave=Date.now();persist().catch(()=>abort.abort());}},abort.signal,{profile,onStatus:message=>emit('status',{message}),
      resolveAttachment:async ref=>(await externalAttachment(env,auth.owner,ref))||bytesFor(env,auth.owner,ref),onUploadStart:async ref=>{await db.prepare("INSERT INTO provider_cleanup(id,owner_id,provider,state,created_at) VALUES(?,?,?,'uploading',?)").bind(runId+':'+ref,auth.owner,b.provider,now()).run();},
    ...
    const xaiPricing=b.provider==='xai'&&!providerCostFields(result).actualCostTicks?await provider.pricingModel('xai',result.servedModel||b.model,'chat').catch(()=>null):null;
    const metadata={...result,text:undefined,files:undefined,artifacts,cleanup:result.cleanup?.length?'Provider file cleanup is pending; recovery will retry.':null};await persist(result.completed?'completed':'partial',metadata);await recordUsage(env,{idempotencyKey:`chat:${runId}`,logicalRequestId:runId,ownerId:auth.owner,projectId:b.projectId,conversationId:runId,provider:b.provider,profileId:credential.profileId,fingerprint:credential.fingerprint,model:b.model,servedModel:result.servedModel,providerRequestId:result.providerRequestId,operation:'research_chat',outcome:result.completed?'succeeded':'partial',...usageFromResponse(result.usage||{}),toolCalls:result.tools?.length||0,...providerCostFields(result),xaiPricing,evidence:{toolTypes:(result.tools||[]).map(tool=>tool.type)},metadata:{sources:result.sources?.length||0,artifacts:artifacts.length}});emit('done',metadata);
  }catch(e){const message=sanitize(e.message,env);await persist(abort.signal.aborted?'stopped':'failed',{warning:message}).catch(()=>{});await recordUsage(env,{idempotencyKey:`chat:${runId}`,ownerId:auth.owner,projectId:b.projectId,conversationId:runId,provider:b.provider,profileId:credential.profileId,fingerprint:credential.fingerprint,model:b.model,operation:'research_chat',outcome:abort.signal.aborted?'stopped':'failed',metadata:{errorCode:e.code||null}}).catch(()=>{});emit('error',{message});}
