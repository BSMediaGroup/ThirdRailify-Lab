...
export function references(value) {
  ...
  function walk(v,key='',depth=0) { if(depth>25||++visits>40000)throw new LabError('Project structure is too complex.');
    if(typeof v==='string') { const match=v.match(/^\/(?:assets|attachments)\/([a-f0-9-]{36}(?:\.(?:png|jpg|webp|gif))?)$/);if(match)found.add(match[1]); if(['assetId'].includes(key)&&v)found.add(v); }
...
export async function rate(env,subject,category) {
  const limits={read:600,upload:40,discovery:80,image:12,chat:30,auth:30,settings:30,image_search:20,image_thumbnail:160,image_import:12,stock_search:30,stock_import:12,usage:60,csp_diagnostic:5};const limit=limits[category];if(!limit)throw new Error('Unknown rate category');const window=Math.floor(Date.now()/60000);
