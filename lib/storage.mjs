...
export async function ready(env) { const db=dbOf(env); const schema=await db.prepare('SELECT version FROM lab_schema WHERE version=1').first(); if(!schema) throw new LabError('Workshop schema is unavailable.',503); return db; }
...
export function references(value) {
  ...
  function walk(v,key='',depth=0) { if(depth>25||++visits>40000)throw new LabError('Project structure is too complex.');
    ...
    else if(Array.isArray(v)){for(const x of v){if(key==='attachments'&&typeof x==='string')found.add(x);else walk(x,key,depth+1);}}
...
export async function storeFile(env,owner,projectId,bytes,metadata={},fixedId=null) {
  ...
  const meta={...metadata,id:fileId,mime:type.mime,bytes:bytes.length,url:(metadata.kind&&metadata.kind!=='image'?'/attachments/':'/assets/')+fileId,createdAt:now()};
  const db=dbOf(env);
...
export async function rate(env,subject,category) {
  const limits={read:600,upload:40,discovery:80,image:12,chat:30,auth:30,settings:20,image_search:20,image_thumbnail:160,image_import:12};const limit=limits[category];if(!limit)throw new Error('Unknown rate category');const window=Math.floor(Date.now()/60000);
