...
export async function rate(env,subject,category) {
  const limits={read:600,upload:40,discovery:80,image:12,chat:30,auth:30,settings:20};const limit=limits[category];if(!limit)throw new Error('Unknown rate category');const window=Math.floor(Date.now()/60000);
