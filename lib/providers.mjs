...
export class Providers {
  constructor(getConfig,fetchImpl=globalThis.fetch,credentials={}){this.config=getConfig;this.fetch=(...args)=>fetchImpl(...args);this.credentials=credentials;this.cache=new Map();}
  key(provider){
    const key=this.credentials[provider]?.secret||this.config()[{replicate:'REPLICATE_API_TOKEN',openai:'OPENAI_API_KEY',xai:'XAI_API_KEY'}[provider]];
    if(!key)throw new LabError(`Add your ${provider==='xai'?'Grok / SpaceXAI':provider} API key in Connections first.`,409,'key_missing');
    return key;
  }
  credential(provider){return this.credentials[provider]||{provider,profileId:`runtime:${provider}`,label:'Runtime Default',fingerprint:null,runtime:true};}
  ...
  async pricingModel(provider,model,kind='chat'){
    if(provider!=='xai'||!/^[a-zA-Z0-9_.:-]{1,180}$/.test(model||''))return null;
    const route=kind==='image'?'/image-generation-models/':'/language-models/';
    try{return await this.request('xai',route+encodeURIComponent(model));}
    catch(error){if(![404,405].includes(error.status))throw error;return this.request('xai','/models/'+encodeURIComponent(model));}
  }
