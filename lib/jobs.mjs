...
export async function enqueue(env,auth,b,provider=null){
  ...
  if(b.provider==='replicate'){parseModel(b.model);const model=await effectiveProvider.model(b.model);snapshot.input=preparePrompt(snapshot.input,model,snapshot.prompt);snapshot.version=model.version;snapshot.validatedModel={id:model.id,version:model.version,official:model.official,schema:model.schema};validateInput(snapshot.input,model.schema);}
  ...
  for(let row of rows.results){const claim=await db.prepare("UPDATE jobs SET lease_until=? WHERE id=? AND lease_until<? AND (SELECT count(*) FROM jobs WHERE lease_until>?)<2").bind(Date.now()+600000,row.id,Date.now(),Date.now()).run();if(!claim.meta.changes)continue;
    try{
      ...
      if(row.status==='queued'){
        ...
        const snapshot=JSON.parse(row.snapshot),credential=await resolveCredential(env,row.owner_id,row.provider,snapshot.model,snapshot.profileId||''),activeProvider=provider||new Providers(()=>env,globalThis.fetch,{[row.provider]:credential});snapshot.profileId=credential.profileId;snapshot.profileFingerprint=credential.fingerprint;snapshot.input=await materialize(env,row.owner_id,snapshot.input);snapshot.options={...snapshot.options,references:await materialize(env,row.owner_id,snapshot.options.references||[])};
        ...
        if(row.provider==='replicate'){
          ...
          await recordUsage(env,{idempotencyKey:`job:${row.id}:provider`,ownerId:row.owner_id,projectId:row.project_id,jobId:row.id,provider:row.provider,profileId:credential.profileId,fingerprint:credential.fingerprint,model:snapshot.model,operation:'image_generation',outcome:prediction.status||'submitted',providerRequestId:prediction.id,generatedOutputs:Array.isArray(prediction.output)?prediction.output.length:null,...usageFromResponse(prediction.metrics||prediction.usage||{}),...providerCostFields(prediction),metadata:{version:snapshot.version||null,predictTime:prediction.metrics?.predict_time??null}});
        }else{
          ...
          await recordUsage(env,{idempotencyKey:`job:${row.id}:provider`,ownerId:row.owner_id,projectId:row.project_id,jobId:row.id,provider:row.provider,profileId:credential.profileId,fingerprint:credential.fingerprint,model:snapshot.model,operation:'image_generation',outcome:'succeeded',providerRequestId:result.id||null,generatedOutputs:result.data.length,...usageFromResponse(result.usage||{}),...providerCostFields(result),metadata:{responseFormat:'private_staged'}});
        }
