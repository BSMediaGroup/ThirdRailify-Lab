...
Updated 11 September 2026 for 0.2.0-poc. This guide concerns the downloadable local POC, not an already hosted service.
...
The image and chat model dropdowns populate from the OpenAI catalogue available to your API key. Refresh reloads that list; no manual model-name entry is required. Connections uses the same catalogues for optional saved defaults. The list supplies IDs rather than complete endpoint capabilities, so specialized models/options may still return a compatibility or access error on submission. Reference: [list models](https://developers.openai.com/api/reference/resources/models/methods/list).
...
Image and chat models populate independently from Grok's typed provider catalogues. Use their dropdowns and refresh icons; the app does not require you to guess IDs. Existing saved .env choices are preserved as preferences, not treated as a guarantee of access. Reference: [model catalogues](https://docs.x.ai/developers/rest-api-reference/inference/models).
...
## 11. Fonts, logo and network behaviour
...
the provider catalogue. The server never lists invented available model names;
...
history. Discard current work is in the canvas toolbar. Deleting a session keeps
