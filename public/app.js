...
function providerDisplayName(provider){return ({replicate:'Replicate',openai:'OpenAI',xai:'Grok / xAI'})[provider]||String(provider||'Unknown provider');}
...
function imageSearchMessage(kind,title,text){$('googleSearchState').className='image-search-state '+kind;$('googleSearchState').innerHTML=`${icon(kind==='loading'?'refresh':kind==='error'?'warning':'image')}<h3>${esc(title)}</h3><p>${esc(text)}</p>`;$('googleSearchState').hidden=false;}
function restoreImageSearchUI(){
  ...
  if(state.googleResults?.sid&&state.googleResults.sid!==state.activeSessionId){state.googleResults=null;$('googleElement').replaceChildren();googleElementName=null;}
  ...
  else if(!state.googleResults||state.googleResults.sid!==state.activeSessionId)imageSearchMessage('empty','Your reference shelf.','Google renders image-first results here with its required branding and advertising.');
}
function normalizeGoogleClientResult(value={}){const image=value.image||{},thumb=value.thumbnailImage||{};return {title:String(value.title||'Untitled image'),url:String(value.url||''),visibleUrl:String(value.visibleUrl||''),contextUrl:String(value.contextUrl||value.url||''),image:{url:String(image.url||value.url||''),width:Number(image.width)||null,height:Number(image.height)||null},thumbnailImage:{url:String(thumb.url||''),width:Number(thumb.width)||null,height:Number(thumb.height)||null},fileFormat:value.fileFormat?String(value.fileFormat):null};}
let googleElementPromise,googleElementName,googleRenderSerial=0;
...
function renderGoogleElement(){if(!window.google?.search?.cse?.element||!state.config?.googleImages?.configured)return;googleElementName='lab-images-'+(++googleRenderSerial);$('googleElement').replaceChildren();window.google.search.cse.element.render({div:'googleElement',tag:'searchresults-only',gname:googleElementName,attributes:googleAttributes()});}
async function loadGoogleSearchElement(){if(!state.config?.googleImages?.configured)return;if(window.google?.search?.cse?.element){if(!googleElementName)renderGoogleElement();return;}if(googleElementPromise)return googleElementPromise;window.__gcse={parsetags:'explicit',initializationCallback:renderGoogleElement,searchCallbacks:{image:{starting(_name,query){const activityId=crypto.randomUUID();state.googleActivity={sid:state.activeSessionId,query,activityId};api('/api/research/images/activity',{method:'POST',body:{projectId:state.activeSessionId,action:'search_started',activityId,query}}).catch(()=>{});return query;},ready(_name,query,_promotions,results){state.googleResults={sid:state.activeSessionId,query,activityId:state.googleActivity?.activityId||crypto.randomUUID(),items:(results||[]).map(normalizeGoogleClientResult)};return false;},rendered(){requestAnimationFrame(attachGoogleActions);}}}};googleElementPromise=new Promise((resolve,reject)=>{const script=document.createElement('script');script.src='https://cse.google.com/cse.js?cx='+encodeURIComponent(state.config.googleImages.cx);script.async=true;script.onload=resolve;script.onerror=()=>reject(new Error('The Google Search Element could not be loaded.'));document.head.append(script);});return googleElementPromise;}
function attachGoogleActions(){const search=state.googleResults;if(!search||search.sid!==state.activeSessionId)return;$('googleSearchState').hidden=Boolean(search.items.length);const cards=$('googleElement').querySelectorAll('.gsc-imageResult');cards.forEach((card,index)=>{const item=search.items[index];if(!item||card.querySelector('.lab-google-actions'))return;const actions=document.createElement('div');actions.className='lab-google-actions';actions.innerHTML=`<button type="button" data-image-action="chat">${icon('chat')}Add to chat</button><button type="button" data-image-action="generator">${icon('image')}Reference</button><button type="button" data-image-action="compose">${icon('layers')}Compose</button><a href="${esc(item.contextUrl)}" target="_blank" rel="noopener noreferrer">${icon('external')}Source</a>`;actions.querySelectorAll('[data-image-action]').forEach(button=>button.onclick=()=>importGoogleResult(item,button.dataset.imageAction,button).catch(e=>toast(e.message,true)));card.append(actions);});}
async function runGoogleSearch(){const sid=state.activeSessionId,q=$('googleQuery').value.trim();if(!sid)throw new Error('Open a project before searching.');if(q.length<2)throw new Error('Enter at least two characters.');saveImageSearchUI();await ensureRemoteProject(sid);await loadGoogleSearchElement();if(!googleElementName)renderGoogleElement();const element=window.google?.search?.cse?.element?.getElement(googleElementName);if(!element)throw new Error('Google Images is still initializing. Try again.');$('googleExternal').href='https://www.google.com/search?tbm=isch&q='+encodeURIComponent(q);imageSearchMessage('loading','Searching Google Images…','Google will render its compliant image results below.');element.execute(q);}
...
on('googleClear','click',()=>{$('googleQuery').value='';$('googleClear').hidden=true;$('googleElement').replaceChildren();state.googleResults=null;saveImageSearchUI();restoreImageSearchUI();$('googleQuery').focus();});
...
function adaptiveUsd(value){if(value==null||!Number.isFinite(Number(value)))return 'Unknown';const amount=Number(value),absolute=Math.abs(amount);if(!absolute)return '$0.00';if(absolute>=1)return '$'+amount.toFixed(2);if(absolute>=.01)return '$'+amount.toFixed(4);if(absolute>=.0001)return '$'+amount.toFixed(6).replace(/0+$/,'');return absolute<.0001?'<$0.0001':'$'+amount.toFixed(8).replace(/0+$/,'');}
...
async function loadUsageDashboard(){if(state.page!=='settings')return;const params=new URLSearchParams({range:$('usageRange').value});if($('usageProvider').value)params.set('provider',$('usageProvider').value);const data=await api('/api/usage?'+params),s=data.summary||{},metric=$('usageMetric').value;
  ...
  $('usageProviders').innerHTML=(data.providers||[]).map(row=>`<div><div class="usage-provider-title">${providerMark(row.provider)}<strong>${esc(providerDisplayName(row.provider))}</strong></div><span>${numberFormat.format(row.requests)} requests · ${numberFormat.format(row.tokens)} tokens</span><small>${row.rate_remaining==null?'Quota evidence unavailable':numberFormat.format(row.rate_remaining)+' of '+numberFormat.format(row.rate_limit)+' reported remaining'}<br>Actual ${Number(row.actual_count||0)?moneyTicks(row.actual_ticks):'none reported'} · estimated ${Number(row.estimated_count||0)?'≈ '+moneyNanos(row.estimated_nanos):'none available'}</small></div>`).join('')||'<div><strong>No provider rows</strong><small>Balances are unavailable unless a supported provider response supplies them.</small></div>';
  $('usageRecent').innerHTML=(data.recent||[]).length?`<table><thead><tr><th>Provider / operation</th><th>Model</th><th>Profile</th><th>Usage</th><th>Outcome</th><th>Cost</th><th>Cost basis / coverage</th></tr></thead><tbody>${data.recent.map(row=>{const cost=costPresentation(row),usage=[row.input_tokens||row.output_tokens?numberFormat.format(Number(row.input_tokens||0)+Number(row.output_tokens||0))+' tokens':'',row.generated_outputs?numberFormat.format(row.generated_outputs)+' outputs':'',row.search_count?numberFormat.format(row.search_count)+' searches':'',row.tool_calls?numberFormat.format(row.tool_calls)+' tools':''].filter(Boolean).join(' · ')||'Not reported';let source='';try{const snapshot=JSON.parse(row.pricing_snapshot_json||'{}');source=snapshot.sourceUrl?`<a href="${esc(snapshot.sourceUrl)}" target="_blank" rel="noopener noreferrer">Official source</a> · ${esc(snapshot.effectiveFrom||'effective date not reported')}`:'';}catch{}return `<tr><td data-label="Provider / operation"><div class="usage-provider-title">${providerMark(row.provider)}<span><strong>${esc(providerDisplayName(row.provider))}</strong><small>${esc(row.operation)}</small></span></div></td><td data-label="Model"><strong>${esc(row.served_model||row.model||'Not reported')}</strong>${row.served_model&&row.model&&row.served_model!==row.model?`<small>Requested ${esc(row.model)}</small>`:''}</td><td data-label="Profile">${esc(row.key_profile_id||'Runtime / legacy')}</td><td data-label="Usage">${esc(usage)}</td><td data-label="Outcome"><span class="usage-outcome">${esc(row.outcome)}</span></td><td data-label="Cost"><strong class="usage-cost ${cost.tone}">${esc(cost.value)}</strong><span class="usage-cost-chip ${cost.tone}">${esc(cost.chip)}</span></td><td data-label="Cost basis"><div class="usage-cost-detail" tabindex="0"><span>${esc((row.coverage_reason||'No pricing evidence retained.').slice(0,120))}</span><div class="usage-cost-popover" role="tooltip"><strong>${esc(cost.chip)} · ${esc(cost.value)}</strong><p>${esc(row.calculation_text||row.coverage_reason||'No calculation is available.')}</p><small>${esc(row.pricing_rate_id||'No pricing rate')} ${source}</small></div></div></td></tr>`;}).join('')}</tbody></table>`:'';
}
