...
const money=micros=>micros==null?'Unknown':'$'+(Number(micros)/1_000_000).toFixed(4);
async function loadUsageDashboard(){if(state.page!=='settings')return;const params=new URLSearchParams({range:$('usageRange').value,metric:$('usageMetric').value});if($('usageProvider').value)params.set('provider',$('usageProvider').value);const data=await api('/api/usage?'+params),s=data.summary||{};$('usageKpis').innerHTML=[['Requests',s.requests],['Tokens',s.tokens],['Outputs',s.outputs],['Searches',s.searches],['Tool calls',s.tool_calls]].map(([label,value])=>`<div><span>${label}</span><strong>${value==null?'Unavailable':numberFormat.format(value)}</strong></div>`).join('')+`<div><span>Actual reported</span><strong>${money(s.actual_micros)}</strong></div><div><span>Estimated</span><strong>${money(s.estimated_micros)}</strong></div><div><span>Unknown cost</span><strong>${s.unknown_count==null?'Unavailable':numberFormat.format(s.unknown_count)+' operations'}</strong></div>`;const metric=$('usageMetric').value,series=data.series||[],max=Math.max(1,...series.map(row=>Number(row[metric])||0));$('usageChart').innerHTML=series.length?`<svg viewBox="0 0 640 180" preserveAspectRatio="none" aria-hidden="true"><path d="${series.map((row,index)=>`${index?'L':'M'} ${series.length===1?320:index*640/(series.length-1)} ${164-(Number(row[metric])||0)*140/max}`).join(' ')}" fill="none" stroke="currentColor" stroke-width="3" vector-effect="non-scaling-stroke"/></svg><div><span>${esc(series[0].day)}</span><strong>${esc(metric.toUpperCase())}</strong><span>${esc(series.at(-1).day)}</span></div>`:'<div class="soft-note">No usage has been recorded for this filter. Missing cost remains unknown, not zero.</div>';$('usageProviders').innerHTML=(data.providers||[]).map(row=>`<div><strong>${esc(row.provider)}</strong><span>${numberFormat.format(row.requests)} requests · ${numberFormat.format(row.tokens)} tokens</span><small>${row.rate_remaining==null?'Quota evidence unavailable':numberFormat.format(row.rate_remaining)+' of '+numberFormat.format(row.rate_limit)+' reported remaining'} · actual ${money(row.actual_micros)} · estimated ${money(row.estimated_micros)}</small></div>`).join('')||'<div><strong>No provider rows</strong><small>Balances are unavailable unless a supported provider response supplies them.</small></div>';$('usageRecent').innerHTML=(data.recent||[]).length?`<table><thead><tr><th>Provider / operation</th><th>Profile</th><th>Outcome</th><th>Cost coverage</th></tr></thead><tbody>${data.recent.map(row=>`<tr><td><strong>${esc(row.provider)}</strong><small>${esc(row.model||row.operation)}</small></td><td>${esc(row.key_profile_id||'Not reported')}</td><td>${esc(row.outcome)}</td><td>${esc(row.cost_state)} · ${money(row.provider_cost_micros??row.estimated_cost_micros)}</td></tr>`).join('')}</tbody></table>`:'';}
