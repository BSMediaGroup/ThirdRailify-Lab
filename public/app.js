...
let toastSequence=0;const toastTimers=new Map(),pendingGenerations=new Set();let sessionSwitchToken=0;
function toastIconNode(name){const svg=document.createElementNS('http://www.w3.org/2000/svg','svg');svg.classList.add('icon');svg.setAttribute('aria-hidden','true');svg.setAttribute('viewBox','0 0 24 24');if(name==='signal'){const path=document.createElementNS('http://www.w3.org/2000/svg','path');path.setAttribute('d','M5 18h14M8 18l4-14 4 14M9.5 12h5M7 7 4 4M17 7l3-3');svg.append(path);}else{const use=document.createElementNS('http://www.w3.org/2000/svg','use');use.setAttribute('href',`/icons.svg#${name}`);svg.append(use);}return svg;}
function dismissToast(card){if(!card)return;clearTimeout(toastTimers.get(card.id));toastTimers.delete(card.id);card.remove();const region=$('toast');region.hidden=!region.children.length;if(region.hidden&&region.parentElement!==document.body)document.body.append(region);}
function toast(message,error=false,options={}){
  const cleanMessage=String(message||'').replace(/\s+/g,' ').trim();if(!cleanMessage)return '';
  const tone=options.tone||(error?'warning':'success'),title=options.title||(tone==='success'?'Update complete':tone==='warning'?'Attention':'Notice'),duration=Math.min(15000,Math.max(2000,options.durationMs||5200)),region=$('toast'),host=document.querySelector('dialog[open]')||document.body;
  if(region.parentElement!==host)host.append(region);for(const duplicate of region.querySelectorAll('.admin-toast'))if(duplicate.querySelector('.admin-toast__copy span')?.textContent===cleanMessage)dismissToast(duplicate);
  const id=`lab-toast-${Date.now()}-${++toastSequence}`,card=document.createElement('section'),iconWrap=document.createElement('span'),copy=document.createElement('span'),heading=document.createElement('strong'),body=document.createElement('span'),dismiss=document.createElement('button'),timer=document.createElement('i');
  card.id=id;card.className=`admin-toast admin-toast--${tone}`;card.setAttribute('role','status');card.setAttribute('aria-atomic','true');card.style.setProperty('--admin-toast-duration',`${duration}ms`);iconWrap.className='admin-toast__icon';iconWrap.setAttribute('aria-hidden','true');iconWrap.append(toastIconNode(tone==='success'?'shield':'signal'));copy.className='admin-toast__copy';heading.textContent=title;body.textContent=cleanMessage;copy.append(heading,body);dismiss.className='admin-toast__dismiss';dismiss.type='button';dismiss.setAttribute('aria-label','Dismiss notification');dismiss.append(toastIconNode('close'));dismiss.addEventListener('click',()=>dismissToast(card));timer.className='admin-toast__timer';timer.setAttribute('aria-hidden','true');card.append(iconWrap,copy,dismiss,timer);region.append(card);while(region.children.length>4)dismissToast(region.firstElementChild);region.hidden=false;toastTimers.set(id,setTimeout(()=>dismissToast(card),duration));return id;
}
document.addEventListener('keydown',event=>{if(event.key==='Escape'&&!document.querySelector('dialog[open]'))dismissToast($('toast')?.lastElementChild);});
