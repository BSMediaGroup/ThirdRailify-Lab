...
test('existing account schema upgrades preserve account rows; fresh Lab schema has independent ownership constraints',async t=>{
 ...
 await apply(lab,new URL('../migrations/0001_lab.sql',import.meta.url));assert.equal((await lab.prepare('SELECT version FROM lab_schema').first()).version,1);await apply(lab,new URL('../migrations/0002_provider_vault_stock_usage.sql',import.meta.url));await apply(lab,new URL('../migrations/0003_provider_cost_intelligence.sql',import.meta.url));await apply(lab,new URL('../migrations/0004_usage_classification_repair.sql',import.meta.url));assert.equal((await lab.prepare('SELECT version FROM lab_schema').first()).version,4);assert.deepEqual((await lab.prepare('PRAGMA foreign_key_check').all()).results,[]);
});
