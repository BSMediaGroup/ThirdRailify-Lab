...
test('existing account schema upgrades preserve account rows; fresh Lab schema has independent ownership constraints',async t=>{
 ...
 const base=new URL('../../ThirdRailify-Admin/migrations/',import.meta.url);
 ...
 await apply(lab,new URL('../migrations/0001_lab.sql',import.meta.url));assert.equal((await lab.prepare('SELECT version FROM lab_schema').first()).version,1);assert.deepEqual((await lab.prepare('PRAGMA foreign_key_check').all()).results,[]);
});
