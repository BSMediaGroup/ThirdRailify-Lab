...
test('real local D1/R2: authority, revisions, isolation, webhook, recovery and fail-closed routes',async t=>{
  ...
  await t.test('encrypted key profiles, stock contracts, 24-hour cache, Unsplash tracking and usage idempotency',async()=>{
    ...
    await recordUsage(env,{idempotencyKey:'usage-fixture',ownerId:'a',provider:'openai',operation:'chat',outcome:'started'});await recordUsage(env,{idempotencyKey:'usage-fixture',ownerId:'a',provider:'openai',operation:'chat',outcome:'succeeded',inputTokens:10,outputTokens:5});const report=await usageDashboard(env,auth('a'),new URL(origin+'/api/usage?range=24h&provider=openai'));assert.equal(report.providers.some(row=>row.tokens===15),true);assert.equal((await env.LAB_DB.prepare("SELECT count(*) n FROM provider_usage_events WHERE idempotency_key='usage-fixture'").first()).n,1);
  });
  let job;
