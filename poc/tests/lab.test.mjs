...
test('provider submission failure is not automatically retried or disguised as a generation',async()=>{
 ...
 try{
  ...
  const port=await app.start(0),base=`http://127.0.0.1:${port}`,init=await(await fetch(base+'/api/state')).json();
  await fetch(base+'/api/generate',{method:'POST',headers:{Origin:base,'Content-Type':'application/json','X-Lab-CSRF':init.csrf},body:JSON.stringify({requestId:'uncertain-1234567890',provider:'openai',prompt:'A test'})});
...
test('Responses chat streams only actual text deltas; no fabricated reasoning',async()=>{
 ...
 try{
  ...
  const port=await app.start(0),base=`http://127.0.0.1:${port}`,init=await(await fetch(base+'/api/state')).json();
  const r=await fetch(base+'/api/chat',{method:'POST',headers:{Origin:base,'Content-Type':'application/json','X-Lab-CSRF':init.csrf},body:JSON.stringify({provider:'openai',messages:[{role:'user',content:'Hello'}]})});const text=await r.text();assert.match(text,/Actual text/);assert.match(text,/event: done/);
 }finally{if(app)await app.close();await rm(root,{recursive:true,force:true});}
});
