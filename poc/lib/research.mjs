...
export function responseBody(provider,model,messages,profile={},fileIds=[]){
  ...
  if(p.maxOutputTokens!==null)b.max_output_tokens=p.maxOutputTokens;
  return b;
}
...
export function responseMetadata(response={}){
  ...
  return {sources:sources.slice(0,60),files:files.slice(0,12),usage:response.usage||null,tools:(response.output||[]).filter(x=>/_(?:call)$/.test(x.type||'')&&x.type!=='function_call').map(x=>({type:x.type,status:x.status||'reported'})).slice(0,30)};
}
...
export async function researchChat(providers,provider,messages,model,onDelta,signal,{profile={},resolveAttachment=async()=>null,onStatus=()=>{}}={}){
  ...
  try{
    ...
    for(const m of messages){
      ...
      for(const id of m.attachments){
        ...
        if(a.kind==='image')content.push({type:'input_image',image_url:`data:${a.mime};base64,${a.bytes.toString('base64')}`,detail:p.imageDetail});
        else{
          ...
          if(!fileId){
            ...
            const res=await providers.fetch(base+'/files',{method:'POST',headers:{Authorization:'Bearer '+key},body:form,signal,redirect:'error'});
            ...
            uploads.set(id,file.id);fileId=file.id;
          }
    ...
    const res=await providers.fetch(base+'/responses',{method:'POST',headers:{Authorization:'Bearer '+key,'Content-Type':'application/json'},body:JSON.stringify(body),signal,redirect:'error'});
    ...
    function consume(line){
      ...
      if(e.type==='response.output_text.delta'&&typeof e.delta==='string'){text+=e.delta;onDelta(e.delta);}
    ...
    for(const fileId of uploads.values()){
      try{const r=await providers.fetch(base+'/files/'+encodeURIComponent(fileId),{method:'DELETE',headers:{Authorization:'Bearer '+key},signal:AbortSignal.timeout(10000),redirect:'error'});if(!r.ok)cleanup.push({fileId,status:r.status});}
